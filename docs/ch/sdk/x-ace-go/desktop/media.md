...
这个接口主要是设置模型文件的根目录，可以将` SDK_DIR/models `作为参数设置。这个接口的主要作用是为了解决了在使用AI功能时要针对不同相机进行设置不同的AI模型的繁琐。
...
`void SetExportFrameSequence(const std::vector <uint64 _t>& vec)`
...
- 曝光： **SetExposure** 范围[-100,100]
- 高光： **SetHighlights**范围[-100,100]
- 阴影： **SetShadows** 范围[-100,100]
- 对比度： **SetContrast** 范围[-100,100]
- 亮度： **SetBrightness** 范围[-100,100]
- 黑点： **SetBlackpoint** 范围[-100,100]
- 饱和度： **SetSaturation** 范围[-100,100]
- 自然饱和度： **SetVibrance** 范围[-100,100]
- 色温： **SetWarmth** 范围[-100,100]
- 色调： **SetTint** 范围[-100,100]
- 清晰度： **SetDefinition** 范围[0,100]
...
可以参考`MediaSDK_ROOT/example/realtime_stitcher_demo.cc`
