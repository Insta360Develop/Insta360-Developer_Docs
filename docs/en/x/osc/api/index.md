...
| `shutterSpeedSupport` | list of supported shutter speed values |
| `hdrSupport` | list of supported hdr values |
| `hdr` | current hdr value |
| `totalSpace` | total SD card space in byte |
...
| `exposureProgram` | exposure mode: 1 = manual, 2 = normal program, 4 = shutter priority, 9 = ISO priority |
| `exposureDelay` | exposure delay time; after starting shooting, delay for a period before exposing |
...
| `whiteBalanceSupport` | list of supported white balance settings |
| `_dateTime` | time format "YYYY:MM:DD HH:MM:SS", e.g. "2022:05:06 17:56:15" |
| `_MuteEnable` | mute setting: 0 = no mute, 1 = mute |
| `_batteryCapacity` | `powerType`: charging status (0 = not charging, 1 = charging); `powerLevel`: battery percentage (100 = 100%, 0 = no battery) |
